...
export default function Layout({ children }: LayoutProps) {
  const appId = typeof window !== 'undefined' ? encodeURIComponent(window.location.hostname) : 'edutech-nova';
        ...
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-1">
            <Link
              ...
              activeProps={{ className: 'px-4 py-2 rounded-lg text-sm font-semibold text-amber bg-amber-light/50' }}
            >
              Quiz
            </Link>
            <Link
              to="/motivation"
              className="px-4 py-2 rounded-lg text-sm font-semibold text-muted-foreground hover:text-foreground hover:bg-accent transition-all duration-150"
              activeProps={{ className: 'px-4 py-2 rounded-lg text-sm font-semibold text-teal bg-teal-light/50' }}
            >
              Motivation
            </Link>
          </nav>
